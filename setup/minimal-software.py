...
inst('keepassxc gnome-screenshot gnome-keyring neofetch volumeicon unclutter-xfixes-git')
...
# Outros
subprocess.run(["sudo", "rmmod", "pcspkr"], stdout=subprocess.DEVNULL,
...
subprocess.run("sudo sed -i 's/^\(load-module\smodule-stream-restore\)$/load-module module-stream-restore restore_device=false/' /etc/pulse/default.pa", shell=True)
